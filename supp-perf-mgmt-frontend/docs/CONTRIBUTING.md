...
Commit messages are validated in the `commit-msg` Git hook using Commitlint.

- Enforced standard: Conventional Commits
- Allowed types: `feat`, `fix`, `chore`, `docs`, `style`, `refactor`, `perf`, `test`, `build`, `ci`
- Any message outside this format is rejected and the commit is not created

Examples rejected by the hook:

```
feature: add summary cards
SPM-104 update quality view
updated styles
```
...
fix: prevent null supplier name render
docs: document commit quality gates
```

### Local pre-commit quality gates

The `pre-commit` hook runs `npm run validate:precommit`, which executes all required checks and blocks commit creation on any failure.

Validation sequence:

1. `npm run lint`
2. `npm run typecheck`
3. `npm run format:check:staged`
4. `npm run test:staged` (affected tests from staged source files)
5. `npm run test:key` (fixed key regression subset from `scripts/key-tests.txt`)

The fixed key test list is stored in `scripts/key-tests.txt`.

- One test file path per line
- Paths are relative to repo root
- Lines beginning with `#` are comments

To update the key suite:

1. Edit `scripts/key-tests.txt` and add/remove paths.
2. Run `npm run test:key` locally.
3. Commit the manifest update with your changes.

If any command fails, the commit is aborted.

### Hook setup and re-setup

Run the following once per clone (or after cleanup):
...
### Troubleshooting local hooks

- Hooks are not firing:
  - Run `npm run prepare` to reinstall Husky hooks.
  - Confirm hook files exist in `.husky/`.
- PowerShell blocks npm scripts:
  - Use `npm.cmd` instead of `npm`.
- `test:staged` says no staged source files:
  - This is expected for docs-only or non-source commits.
- Commit message rejected:
  - Use one of the allowed types and the format `{type}: {description}`.
- Pre-commit too slow:
  - Keep commits focused and stage only related files to reduce affected test scope.
- `test:key` fails due to missing files:
  - Remove stale entries or fix paths in `scripts/key-tests.txt`.
