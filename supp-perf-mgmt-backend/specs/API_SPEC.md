...
Backend A serves all KPI data, authentication, supplier access, and saved filters. **Chat endpoints are not here** — they are served by Backend B (separate repo).
...
> **Chat endpoints** use a separate base URL: `NEXT_PUBLIC_CHAT_API_BASE_URL` (Backend B — separate repo).
...
>
> The full chat endpoint contract (`POST /chat/message`, `POST /chat/session/new`) lives in the Backend B repo.
